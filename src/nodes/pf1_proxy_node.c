									   ...
									   uint16_t nb_objs)
{
	...
	if (sent_count != nb_objs) {
		...
		dp_graphtrace_next_burst(node, objs+sent_count, nb_objs-sent_count, PF1_PROXY_NEXT_DROP);
		rte_node_enqueue(graph, node, PF1_PROXY_NEXT_DROP, objs+sent_count, nb_objs-sent_count);
	}
