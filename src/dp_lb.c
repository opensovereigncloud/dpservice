...
int dp_create_lb(struct dpgrpc_lb *lb, const uint8_t *ul_ip)
{
	...
	if (!DP_FAILED(rte_hash_lookup(ipv4_lb_tbl, &lb_key))) {
		if (!DP_FAILED(rte_hash_lookup_data(ipv4_lb_tbl, &lb_key, (void **)&lb_val)))
...
static int dp_lb_rr_backend(struct lb_value *val, const struct lb_port *lb_port, uint32_t src_ip, rte_be16_t src_port)
{
	int ret = -1, k;
...
uint8_t *dp_lb_get_backend_ip(struct flow_key *flow_key, uint32_t vni, uint32_t src_ip, rte_be16_t src_port)
{
	struct lb_value *lb_val = NULL;
