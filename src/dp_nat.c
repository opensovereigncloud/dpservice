...
int dp_remove_network_snat_port(const struct flow_value *cntrack)
{
	...
	if (unlikely(flow_key_reply->l3_dst.is_v6 || flow_key_org->l3_dst.is_v6)) {
		DPS_LOG_ERR("NAT flow key with IPv6 addresses");  // TODO log?
		return DP_ERROR;
	}

	portoverload_tbl_key.nat_ip = flow_key_reply->l3_dst._data[0];
	portoverload_tbl_key.dst_ip = flow_key_org->l3_dst._data[0];
