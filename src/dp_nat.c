...
int dp_allocate_network_snat_port(struct snat_data *snat_data, struct dp_flow *df, uint32_t vni)
{
	...
	if (df->l3_type == RTE_ETHER_TYPE_IPV4) {
		portmap_key.src_ip.is_v6 = false;
...
int dp_remove_network_snat_port(const struct flow_value *cntrack)
{
	...
	portmap_key.src_ip.is_v6 = flow_key_org->is_v6;
	if (flow_key_org->is_v6)
		rte_memcpy(portmap_key.src_ip.ipv6, flow_key_org->l3_src.ip6, sizeof(portmap_key.src_ip.ipv6));
	else
		portmap_key.src_ip.ipv4 = flow_key_org->l3_src.ip4;
