	...
	if (DP_TCP_PKT_FLAG_RST(tcp_flags)) {
		...
		case DP_FLOW_TCP_STATE_RST_FIN:
			if (DP_TCP_PKT_FLAG_ONLY_SYN(tcp_flags)) {
				...
				flow_val->l4_state.tcp.syn_flow_dir = df->flow_dir;
			}
			break;
		case DP_FLOW_TCP_STATE_NEW_SYN:
			if (DP_TCP_PKT_FLAG_ONLY_SYNACK(tcp_flags) && df->flow_dir != flow_val->l4_state.tcp.syn_flow_dir)
				flow_val->l4_state.tcp.state = DP_FLOW_TCP_STATE_NEW_SYNACK;
			break;
		case DP_FLOW_TCP_STATE_NEW_SYNACK:
			if (DP_TCP_PKT_FLAG_ONLY_ACK(tcp_flags) && df->flow_dir == flow_val->l4_state.tcp.syn_flow_dir)
