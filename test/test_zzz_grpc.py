...
# TODO go through this once agin and test all errors being hit everywhere
...
def test_grpc_interface(prepare_ifaces, grpc_client):
	...
	vmspec = { "vni": VM4.vni, "device": VM4.pci, "primary_ipv4": VM4.ip, "primary_ipv6": VM4.ipv6, "underlay_route": vm4_ul_ipv6 }
...
def test_grpc_route_errors(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(206).addroute(vni3, "0.0.0.0/0", 0, "::")
...
def test_grpc_prefix_errors(prepare_ifaces, grpc_client):
	# Try to address a machine which doesnt exist
...
def test_grpc_neighnat_errors(prepare_ifaces, grpc_client):
	...
	grpc_client.addneighnat(nat_vip, vni2, nat_neigh_min_port, nat_neigh_max_port, neigh_vni1_ul_ipv6)
	# TODO not failing atm.
	grpc_client.delneighnat(nat_vip, vni2, nat_neigh_min_port, nat_neigh_max_port)
	# Try to add an overlapping port range for the same IP
	grpc_client.addneighnat(nat_vip, vni2, nat_neigh_min_port+1, nat_neigh_max_port+1, neigh_vni1_ul_ipv6)
	# TODO not failing atm.
	grpc_client.delneighnat(nat_vip, vni2, nat_neigh_min_port+1, nat_neigh_max_port+1)
	grpc_client.delneighnat(nat_vip, vni1, nat_neigh_min_port, nat_neigh_max_port)
